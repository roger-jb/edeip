...
class CarnetLiaison {
	...
	public static function getByEleves($arrayEleves){
		$query = "SELECT * FROM CARNET_LIAISON WHERE idEleve IN (0";
		foreach ($arrayEleves as $eleve){
			$query .= ', '.$eleve->getIdEleve();
		}
		$query .=") ORDER BY dateRedaction DESC";
		$result = db_connect::query($query);
		$return = array();
		while ($info = $result->fetch_object('CarnetLiaison')){
			$return[] = $info;
