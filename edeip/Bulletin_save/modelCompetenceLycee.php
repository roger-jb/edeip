...
function niveauCpt($idNivCpt){
	...
	switch ($idNivCpt){
		case 1:
			echo '
			<td class="ptCpt nivCpt">X</td>
			<td class="ptCpt nivCpt"></td>
			<td class="ptCpt nivCpt"></td>
			<td class="ptCpt nivCpt"></td>';
			break;
		case 2:
		...
		foreach ($Domaines as $domaine){
			...
			foreach ($pointCpts as $ptCpt) {
				...
				if (!is_null($EPT->getIdNiveauCpt())) {
					echo '<tr>
						<td colspan="4"></td>
						<td class="ptCpt">'.$EPT->getPointCpt()->getLibellePointCpt().'</td>';
...
<page backtop="30mm" backbottom="7mm" backleft="10mm" backright="10mm">
	...
	<table cellspacing="0" style="width: 100%; margin: 0; padding: 0;" class="tableau">
		...
		<tr>
			<td></td>
			<td class="sousCulture" colspan="4">Science &Eacute;conomiques et Sociales</td>
			<td class="nivCpt"></td>
			<td class="nivCpt"></td>
			<td class="nivCpt"></td>
			<td class="nivCpt"></td>
		</tr>
		<?php afficheDomaine($eleve, $trimestre, 14); ?>
		<tr>
			<td></td>
