...
' . $content . '
		...
		protected function _setNewPositionForNewLine ($curr = null) {
			...
			if ($this->parsingCss->value['text-align'] == 'center') $this->pdf->setX(($rx + $this->pdf->getX() - $w) * 0.5 - 0.01);
			else if ($this->parsingCss->value['text-align'] == 'right') $this->pdf->setX($rx - $w - 0.01);
			else
				$this->pdf->setX($lx);
		...
		protected function _makeBreakLine ($h, $curr = null) {
			if ($h) {
				if (($this->pdf->getY() + $h < $this->pdf->getH() - $this->pdf->getbMargin()) || $this->_isInOverflow || $this->_isInFooter) $this->_setNewLine($h, $curr);
				else
		...
		protected function _drawCurve ($pt, $color) {
			...
			if (count($pt) == 10) $this->pdf->drawCurve($pt[0], $pt[1], $pt[2], $pt[3], $pt[4], $pt[5], $pt[6], $pt[7], $pt[8], $pt[9]);
			else
		...
		protected function _tag_open_WRITE ($param) {
			...
			if ($this->parsingCss->value['text-transform'] != 'none') {
				if ($this->parsingCss->value['text-transform'] == 'capitalize') $txt = ucwords($txt);
				else if ($this->parsingCss->value['text-transform'] == 'uppercase') $txt = strtoupper($txt);
				else if ($this->parsingCss->value['text-transform'] == 'lowercase') $txt = strtolower($txt);
			}
		...
		protected function _tag_close_TABLE ($param) {
			...
			if ($this->_subPart) {
				...
				if ($parentWidth > $w) {
					if (HTML2PDF::$_tables[$param['num']]['align'] == 'center') $x = $x + ($parentWidth - $w) * 0.5;
					else if (HTML2PDF::$_tables[$param['num']]['align'] == 'right') $x = $x + $parentWidth - $w;
				...
				if (count(HTML2PDF::$_tables[$param['num']]['height']) > 1) $y = $this->_margeTop + HTML2PDF::$_tables[$param['num']]['height'][count(HTML2PDF::$_tables[$param['num']]['height']) - 1];
				else if (count(HTML2PDF::$_tables[$param['num']]['height']) == 1) $y = HTML2PDF::$_tables[$param['num']]['curr_y'] + HTML2PDF::$_tables[$param['num']]['height'][count(HTML2PDF::$_tables[$param['num']]['height']) - 1];
				else
					$y = HTML2PDF::$_tables[$param['num']]['curr_y'];
				...
				if (isset(HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {
					...
					if (count($colParam['style']['width'])) {
						...
						foreach ($colParam['style']['width'] as $width) {
							if (substr($total, -1) == '%' && substr($width, -1) == '%') $total = (str_replace('%', '', $total) + str_replace('%', '', $width)) . '%';
							else
