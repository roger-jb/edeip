...
class HTML2PDF_parsingCss {
	...
	public function fontSet () {
		...
		if ($this->value['background']['color']) $this->_pdf->setFillColorArray($this->value['background']['color']);
		else
	...
	public function convertBackgroundImage ($css) {
		...
		else if (preg_match('/^url\(([^)]*)\)$/isU', $css, $match)) return $match[1];
		else
			return null;
	}
	...
	protected function _analyseStyle (&$code) {
		...
		for ($k = 0; $k < count($match[0]); $k++) {
			...
			foreach ($names as $name) {
				...
				// save the value
				if (!isset($this->css[$name])) $this->css[$name] = $css;
				else
